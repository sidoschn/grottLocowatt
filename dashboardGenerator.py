...
def generateDashboard(definedkey, deviceid, jsondate):
    try:
        ...
        if (definedkey["bdconoffstate"] == 1 or definedkey["bdconoffstate"] == 3): 
            newSection["cards"].append({"type":"gauge", "entity":sensorNameTag+"pbdc1chrdischr", "name":"Batterie 1 Ladeleistung", "grid_options":{"columns":6,"rows":"auto"}, "severity":{"green":0,"yellow":-definedkey["opfullwatt"],"red":(-definedkey["opfullwatt"]-10)}, "max":(definedkey["opfullwatt"]/3), "min":-(definedkey["opfullwatt"]/3), "needle":"true"}) #power exported to grid
        
        if (definedkey["bdconoffstate"] > 1): 
            newSection["cards"].append({"type":"gauge", "entity":sensorNameTag+"pbdc2chrdischr", "name":"Batterie 2 Ladeleistung", "grid_options":{"columns":6,"rows":"auto"}, "severity":{"green":0,"yellow":-definedkey["opfullwatt"],"red":(-definedkey["opfullwatt"]-10)}, "max":(definedkey["opfullwatt"]/3), "min":-(definedkey["opfullwatt"]/3), "needle":"true"}) #power exported to grid
